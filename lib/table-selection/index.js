...
module.exports = function(app, models, STATICS, bodyParser, itemTypes) {
  ...
  /**
   * Prepares json file for sending to front-end by removing any extra fields
   */
  function fixRowAttributes(documents) {
    if (!documents) {
      documents = [];
    }
    var dI, document, keys, keyI, parenLength, tempKey;
    for (dI = 0; dI < documents.length; dI++) {
      document = documents[dI];
      deleteKey(document, 'rpg');
      deleteKey(document, '_id');
      deleteKey(document, '__v');
    }
  }
  ...
  app.get(STATICS.routes.getAllItemsOfType + "/:itemType", function(req, res) {
    ...
    // lean() is used to return an object, not a model
    // SOURCE: http://stackoverflow.com/questions/7503450/how-do-you-turn-a-mongoose-document-into-a-plain-object
    models[itemType].find({}).lean().exec(function (err, documents) {
      if (err) {
